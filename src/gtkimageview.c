 ...
 *   </para>
 *   <para>
 *     Note that the widget must be given keyboard focus otherwise the
 *     keybindings wont work:
 *     <informalexample>
 *       <programlisting>
 *         // Give GtkImageView keyboard focus.
 ...
 *   </itemizedlist>
 *   <para>
 *     For library users, the most practical coordinate systems are
 *     the image and widget coordinate systems. To convert a widget
 *     coordinate to image, use the following algorithm.
 *     <example><title>Conversion from widget to image space</title>
 *       <programlisting>
 *                                     Aw - Iw*z
 *                          Ox - max ( --------- , 0) + Ex
 *                                         2
 ...
 *       </programlisting>
 *     </example>
 *     The coordinates y-axis works the same way. Conversion the other
 *     way works like this:
 *     <example><title>Conversion from image to widget space</title>
 *       <programlisting>
 *                                        Aw - Iw*z
 *         to_widget_x(Ix) = z*Ix + max ( --------- , 0 ) - Ox
 *                                            2
 *       </programlisting>
 *     </example>
 *   </para>
 ...
 *   </para>
 *   <itemizedlist>
 *     <listitem><emphasis>Ox, Oy</emphasis> : The views offset in zoom space
 *     coordinates. See gtk_image_view_get_viewport().</listitem>
 *     <listitem><emphasis>Aw, Ah</emphasis> : Widget allocation.</listitem>
 *     <listitem><emphasis>Iw, Ih</emphasis> : Width and height of image.</listitem>
 *     <listitem><emphasis>z</emphasis> : The views zoom factor. See
 *     gtk_image_view_get_zoom().</listitem>
 *   </itemizedlist>
 *   <para>
 *     For convenience sake, these algorithms are implemented by
 *     gtk_image_view_widget_to_image_rect() and
...
/**
 * gtk_image_view_widget_to_image_rect:
 * @view: a #GtkImageView
 * @rect_in: a #GdkRectangle in widget space coordinates to convert
 * @rect_out: a #GdkRectangle to fill in with the image space
 *   coordinates
 * @returns: %TRUE if the conversion was successful, %FALSE otherwise
 *
 * Convert a rectangle in widget space coordinates to image space
 * coordinates. If the view is not realized, or if it contains no
 * pixbuf, then the conversion was unsuccessful. %FALSE is then
 * returned and @rect_out is left unmodified.
 *
 * The size of the output rectangle is rounded up so if the widget
 * rectangle crosses a pixel, that pixel is included.
 *
 * Note that the resulting rectangle is not clipped which means that
 * its area may lay outside the pixbuf.
 **/
gboolean
                                     ...
                                     GdkRectangle *rect_out)
{
    ...
    int x = viewport.x - image_rect.x + rect_in->x;
    int y = viewport.y - image_rect.y + rect_in->y;
    gdouble zoom = gtk_image_view_get_zoom (view);
    rect_out->x = (int) ((gdouble) x / zoom);
    rect_out->y = (int) ((gdouble) y / zoom);

    rect_out->width = ceil ((gdouble) rect_in->width / zoom);
    rect_out->height = ceil ((gdouble) rect_in->height / zoom);
    return TRUE;
}
